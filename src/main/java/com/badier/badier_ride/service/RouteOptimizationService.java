...
public class RouteOptimizationService {
    ...
    public Route optimizeRoute(Long routeId) {
        ...
        // Si des points restent à optimiser, utiliser l'API Google Maps ou l'algorithme
        // de fallback
        ...
        for (DeliveryPoint point : points) {
            ...
            try {
                ...
                double distance = mapsApiService.calculateHaversineDistance(
                        latitude, longitude,
                        point.getAddress().getLatitude(),
                        point.getAddress().getLongitude()) * 1000; // Convertir km en mètres
            ...
            if (currentLocation != null) {
                try {
                    ...
                    double distanceInMeters = mapsApiService.calculateHaversineDistance(
                            currentLocation.getLatitude(), currentLocation.getLongitude(),
                            point.getAddress().getLatitude(), point.getAddress().getLongitude()) * 1000; // convertir km
                                                                                                         // en mètres
    ...
    /**
     * Optimise l'ordre des points de livraison en respectant les points de départ
     * et d'arrivée définis
     */
    @Transactional
    public Route optimizeRouteWithFixedPoints(Long routeId) {
        ...
        // Construire la liste finale avec le point de départ, les points optimisés et
        // le point d'arrivée
